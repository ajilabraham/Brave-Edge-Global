...
export const Section2ScrollNarrative: React.FC<Section2ScrollNarrativeProps> = ({ scrollProgress }) => {
  ...
  return (
    <div className="absolute inset-0 z-20 pointer-events-none flex items-center justify-start select-none">
      <div className="max-w-[1720px] w-full mx-auto px-6 sm:px-10 lg:px-14 relative h-full flex items-center">
        
        {/* LEFT COLUMN: Stepper Narrative Container (Constrained width max-w-[440px] to NEVER overlay character) */}
        <div className="relative max-w-[360px] sm:max-w-[420px] lg:max-w-[460px] space-y-10 sm:space-y-14 pl-12 sm:pl-16">
          
          {/* Vertical Stepper Background Track */}
          <div className="absolute top-4 bottom-4 left-4 sm:left-5 w-0.5 bg-neutral-200/70 rounded-full" />
          
          {/* Active Gradient Line Fill */}
          <div
            className="absolute top-4 left-4 sm:left-5 w-0.5 bg-gradient-to-b from-[#F27C23] via-[#58548C] to-[#F27C23] rounded-full transition-all duration-150 ease-out"
          ...
          >
            {/* Step 1 Circle Icon Badge with Dynamic Moving Flash Ring */}
            <div
              className={`absolute -left-[44px] sm:-left-[50px] top-0.5 w-10 h-10 rounded-full bg-white border-2 transition-all duration-300 flex items-center justify-center shrink-0 z-10 ${
                activeStep === 1
            ...
            >
              ...
              {activeStep === 1 && (
                <span className="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-[#F27C23] animate-ping" />
              )}
            </div>

            {/* Step 1 Text Content */}
            <div className="space-y-1">
              <span className="text-xs sm:text-sm font-extrabold tracking-wider text-[#F27C23] uppercase block">
                01
              </span>
              <h3 className="text-xl sm:text-2xl lg:text-3xl font-extrabold text-neutral-900 tracking-tight leading-snug">
                Your intelligence{' '}
          ...
          >
            {/* Step 2 Circle Icon Badge with Dynamic Moving Flash Ring */}
            <div
              className={`absolute -left-[44px] sm:-left-[50px] top-0.5 w-10 h-10 rounded-full bg-white border-2 transition-all duration-300 flex items-center justify-center shrink-0 z-10 ${
                activeStep === 2
            ...
            >
              ...
              {activeStep === 2 && (
                <span className="absolute -top-1 -right-1 w-3 h-3 rounded-full bg-[#58548C] animate-ping" />
              )}
            </div>

            {/* Step 2 Text Content */}
            <div className="space-y-1">
              <span className="text-xs sm:text-sm font-extrabold tracking-wider text-[#58548C] uppercase block">
                02
              </span>
              <h3 className="text-xl sm:text-2xl lg:text-3xl font-extrabold text-neutral-900 tracking-tight leading-snug">
          ...
          >
            {/* Step 3 Circle Icon Badge with Dynamic Moving Flash Ring */}
            <div
              className={`absolute -left-[44px] sm:-left-[50px] top-0.5 w-10 h-10 rounded-full bg-white border-2 transition-all duration-300 flex items-center justify-center shrink-0 z-10 ${
                activeStep === 3
                  ? 'border-[#F27C23] text-[#F27C23] shadow-lg shadow-orange-500/30 ring-4 ring-orange-500/20 scale-110'
        ...
        {leoT > 0.02 && (
          <div
            className="absolute right-6 sm:right-12 lg:right-16 xl:right-24 top-[54%] -translate-y-1/2 z-30 pointer-events-auto transition-all duration-500 ease-out"
            style={{
              opacity: leoOpacity,
              transform: `translateY(calc(-50% + ${leoY}px)) scale(${leoScale})`
            }}
          >
            <a
              href="#chat-leo"
              className="group relative inline-flex items-center gap-2.5 bg-white/95 hover:bg-white backdrop-blur-xl text-neutral-900 px-4 py-2 rounded-full shadow-lg shadow-orange-500/15 hover:shadow-xl hover:shadow-orange-500/25 border border-orange-200/90 hover:border-[#F27C23] transition-all duration-300 transform hover:-translate-y-0.5 hover:scale-105"
            >
              {/* Cute Orange AI Avatar Node with Pulse Dot */}
              <div className="relative w-7 h-7 rounded-full bg-gradient-to-tr from-[#F27C23] to-[#EB893D] text-white flex items-center justify-center shrink-0 shadow-xs">
              ...
              <span className="text-xs sm:text-sm font-bold text-neutral-800 group-hover:text-[#F27C23] transition-colors tracking-wide">
                Talk to Leo
              </span>

              {/* Small Subtle Arrow */}
              <ArrowRight className="w-3.5 h-3.5 text-[#F27C23] group-hover:translate-x-0.5 transition-transform" />
            </a>
          </div>
        )}
